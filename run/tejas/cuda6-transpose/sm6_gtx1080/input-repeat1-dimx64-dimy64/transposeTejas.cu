...
// Compute the tile size necessary to illustrate performance cases for SM12+
// hardware
int MAX_TILES_SM12 = (FLOOR(MATRIX_SIZE_X, 512) * FLOOR(MATRIX_SIZE_Y, 512)) /
                     (TILE_DIM * TILE_DIM);
// Compute the tile size necessary to illustrate performance cases for SM10,SM11
// hardware
int MAX_TILES_SM10 = (FLOOR(MATRIX_SIZE_X, 384) * FLOOR(MATRIX_SIZE_Y, 384)) /
                     (TILE_DIM * TILE_DIM);
...
void getParams(int argc, char **argv, cudaDeviceProp &deviceProp, int &reps, int &size_x,
  ...
  if (checkCmdLineFlag(argc, (const char **)argv, "repeat")) {
    reps = getCmdLineArgumentInt(argc, (const char **)argv, "repeat");
  }
}
...
void showHelp() {
  ...
  printf("\t-repeat=num_repetitions (number of times to run each kernel for measurement)\n");
...
// main
// ----
int GPUTejas_main ( int argc, char **argv ) {
  // Start logs
  printf("%s Starting...\n\n", sSDKsample);
  ...
  for (int k = 0; k < 8; k++) {
    ...
    printf("transpose %s, Throughput = %.4f GB/s, Time = %.5f ms, Size = %u "
           ...
           kernelName, kernelBandwidth, kernelTime / reps,
